...
class TextProcessor:
    def __init__(self):
        ...
        self.ordinals = {
            0: "zeroth", 1: "first", 2: "second", 3: "third", 4: "fourth", 5: "fifth", 6: "sixth",
        ...
        self.scales = { 'crore': 10000000, 'lakh': 100000, 'thousand': 1000, 'hundred': 100}
        ...
        self.letter_prefixes = {
            'Q': 'Quarter', 'P': 'Phase', 'V': 'Version', 'Ch': 'Chapter', 'Fig': 'Figure',
            'Sec': 'Section', 'App': 'Appendix', 'Vol': 'Volume', 'Pg': 'Page', 'Rev': 'Revision',
            'ID': 'ID', 'No': 'Number', 'Ref': 'Reference', 'Table': 'Table', 'Type': 'Type',
            'Level': 'Level', 'Grade': 'Grade', 'Stage': 'Stage', 'Step': 'Step', 'Part': 'Part'
        }

        self.measurement_units = {
            "°C": "degrees celsius", "°F": "degrees fahrenheit", "m": "metres", "cm": "centimetres",
            "mm": "millimetres", "km": "kilometres", "g": "grams", "kg": "kilograms", "mg": "milligrams",
            "l": "litres", "ml": "millilitres", "B": "bytes", "KB": "kilobytes", "MB": "megabytes", 
            "GB": "gigabytes", "TB": "terabytes", "Hz": "hertz",  "kHz": "kilohertz", "MHz": "megahertz", 
            "GHz": "gigahertz", "m²": "square metres", "cm²": "square centimetres", "mm²": "square millimetres"
        }

        self.abbreviations = {
            r"\bMr\.?(?=\s|$|[,;:])": "Mister", r"\bMrs\.?(?=\s|$|[,;:])": "Misses",
            r"\bMs\.?(?=\s|$|[,;:])": "Miss", r"\bDr\.?(?=\s|$|[,;:])": "Doctor",
            r"\bProf\.?(?=\s|$|[,;:])": "Professor", r"\bSr\.?(?=\s|$|[,;:])": "Senior",
            r"\bJr\.?(?=\s|$|[,;:])": "Junior", r"\bSt\.?(?=\s|$|[,;:])": "Saint",
            r"\bRev\.?(?=\s|$|[,;:])": "Reverend", r"\bSgt\.?(?=\s|$|[,;:])": "Sergeant",
            r"\bCol\.?(?=\s|$|[,;:])": "Colonel", r"\bGen\.?(?=\s|$|[,;:])": "General",
    ...
    def _handle_roman_numeral(self, match: re.Match) -> str:
        ...
        # Here, you can decide if you want to output an ordinal version
        # or a cardinal (nominal) version. For now, we'll assume cardinal.
    ...
    def _process_scale_ordinal(self, word: str) -> str:
        ...
        scale_ordinals = { "hundred": "hundredth", "thousand": "thousandth", "lakh": "lakh", "crore": "crore" }
...
def split_sentences(text):
    ...
    like Dr., Mr., Sh., etc.
    """
    # Define abbreviations and temporary placeholders
    abbreviations = [
    ...
    for abbr, placeholder in placeholder_map.items():
        text = text.replace(abbr, placeholder)

    # Now split sentences normally
    sentences = re.split(r"(?<=[.!?])\s+", text)

    # Revert placeholders back to original abbreviations
    restored_sentences = []
    for sentence in sentences:
        for placeholder, abbr in placeholder_map.items():
            sentence = sentence.replace(abbr, placeholder)
...
def main():
    ...
    test_cases = split_sentences(text)
    processed_sentences = []

    for test in test_cases:
        test = test.strip()  # Remove any leading/trailing spaces
        if test:  # Only process non-empty sentences
            print(f"Original: {test}")
            processed_text = processor.process_text(test).strip()
